...
document.addEventListener('DOMContentLoaded', () =>{
  ...
  let worldSettingsModal = document.getElementById('worldSettingsModalContainer')
  let worldSettingsModalButton = document.getElementById('worldSettingsModalBtn')
  let worldSettingsCloseSpan = document.getElementById('worldSettingsModalClose')

  worldSettingsModalButton.onclick = function() {
    worldSettingsModal.style.display = "block"
  }

  worldSettingsCloseSpan.onclick = function() {worldSettingsModal.style.display = "none"}

  window.addEventListener("click", function(event) {
    if (event.target == worldSettingsModal) {
      worldSettingsModal.style.display = "none";
    }
  })



  let lifeSettingsModal = document.getElementById('lifeSettingsModalContainer')
  let lifeSettingsModalButton = document.getElementById('lifeSettingsModalBtn')
  let lifeSettingsCloseSpan = document.getElementById('lifeSettingsModalClose')

  lifeSettingsModalButton.onclick = function() {
    lifeSettingsModal.style.display = "block"
  }

  lifeSettingsCloseSpan.onclick = function() {lifeSettingsModal.style.display = "none"}

  window.addEventListener("click", function(event) {
    if (event.target == lifeSettingsModal) {
      lifeSettingsModal.style.display = "none";
    }
  })


  let acknowledgmentsModal = document.getElementById('acknowledgmentsModalContainer')
  let acknowledgmentsModalButton = document.getElementById('acknowledgmentsModalBtn')
  let acknowledgmentsCloseSpan = document.getElementById('acknowledgmentsModalClose')

  acknowledgmentsModalButton.onclick = function() {
    acknowledgmentsModal.style.display = "block"
  }

  acknowledgmentsCloseSpan.onclick = function() {acknowledgmentsModal.style.display = "none"}
...
class Herbivore extends __WEBPACK_IMPORTED_MODULE_0__lifeform__["a" /* default */] {
  ...
  checkStarvation() {
    ...
    if (this.size < 1 && this.energy < -10) {
// Should be
//    if (this.size === 1 && this.energy < -10) {
// But I am having a bug with starvation and it is too late before the
// showcase to fix it!
      this.die() }
  }
  ...
  canGrow() {
    ...
    // Should be calling super method FixMe
    return (this.size < this.constructor.maxSize &&
            this.turnsSinceSpawnOrGrowth >= this.constructor.waitAfterSpawnOrGrowth
           && this.energy > 5)
  }
...
//    this.tick()
  ...
  inaugurateWorld(world) {
    ...
    let refreshID = window.setInterval(() => {this.tick()},  125)
